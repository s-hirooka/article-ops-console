...
LIFEHOUSE = {
    ...
    "article_structure": (
        ...
        "3) <h2>まず結論｜早見表</h2> … <table> 3列（商品名(Amazonリンク) / 価格帯の目安 / 特徴）で "
        "3〜5 商品\n"
        "4) <h2>商品ごとの詳細</h2> … 商品ごとに <h3>1. 商品名</h3> を作り、"
        "画像（Amazonリンクで囲む）→ 2〜3 文の説明 → <ul> で「価格帯」「こんな人に」→ "
        "<p><a href=...>Amazonで詳細を見る →</a></p>\n"
    ...
    "product_block_spec": (
        "商品は Amazon の売れ筋から手動想定で 3〜5 点。リンクは "
        "https://www.amazon.co.jp/dp/<ASIN>?tag=swork_seo-22 の形式（アソシエイトタグ "
        "swork_seo-22 を必ず付与）。価格は「〇〇円台〜」と幅で書き、断定しない。"
        "画像は <figure class=\"wp-block-image size-full\"> でAmazonリンクを付け、"
        "width:250px 中央寄せ。実機レビューではなく公開仕様・ランキングの比較である前提。"
