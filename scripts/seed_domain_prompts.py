...
_INTERNAL_LINK = (
    "【重要】他記事のURL・パーマリンクを絶対に自分で作らない・推測しない。日付やスラッグを"
    "でっちあげると、公開後にリンク切れになる（Amazon商品のASINを推測しないのと同じ理由）。\n\n"
    "本文末尾に「あわせて読みたい」の見出しを作り、<ul> の中に <li>[[RELATED_ARTICLE]]</li> を"
    "2〜3個並べる（タイトルやURLは書かない — 公開前の自動処理が同ドメインの実在記事の中から"
    "テーマの近いものに差し替える）。"
)
_CANNIBALIZATION = (
...
LIFEHOUSE = {
    ...
    "product_block_spec": (
        ...
        "  [[PRODUCT_TITLE:<検索に使える具体的な商品カテゴリ>]] … 商品名＋実在リンクに置き換わる\n"
        "  [[PRODUCT_PRICE:<同じ検索語>]] … 実勢価格に置き換わる（自分で価格を書かない）\n"
        "  [[PRODUCT_BLOCK:<同じ検索語>]] … 商品画像＋実在リンクのブロックに置き換わる\n\n"
        "同じ商品を指すトークンは検索語（コロンの後ろ）を一字一句同じ文字列にする"
        "（表記が違うと別商品として扱われてしまう）。\n\n"
        "検索語はAmazon内のキーワード検索にそのまま渡され、語をすべて含む商品だけがヒットする"
        "ため、条件を積みすぎると1件もヒットしない（該当ゼロで記事に「商品が見つからない」"
        "注記が残る）。商品カテゴリ＋特徴1つ、計2〜3語までに抑える:\n"
        "  悪い例:「パナソニック対応 ドアホン交換用レンズ 曇り対策」"
        "（ブランド名＋用途＋機能を積み過ぎ／そもそも単体で売っていない部品）\n"
        "  悪い例:「ワイヤレスドアベル 工事不要 磁石取付 賃貸」（条件4つで該当ゼロになりやすい）\n"
        "  良い例:「浴室鏡 曇り止め」「ワイヤレス ドアホン 工事不要」（商品カテゴリ＋"
        "特徴1つ、Amazonで実際に売られている一般名詞の組み合わせ）\n"
        "また、そのトラブルに直結する具体的な商品が存在しない話題（例: 部品の調整方法そのもの）"
        "では、無理に5点そろえようとせず、実在する関連消耗品・工具・保護グッズに絞って"
        "3点程度に減らしてよい。vc_auto_ads ショートコードはこのサイトでは使わない。"
    ),
