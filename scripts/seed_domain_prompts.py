...
LIFEHOUSE = {
    ...
    "article_structure": (
        ...
        "3) <h2>まず結論｜早見表</h2> … <table> 3列（商品カテゴリ / 価格帯の目安 / 特徴）で "
        "3〜5 商品分の product_slot（product_block_spec のルールに従う。実在リンクは書かない）\n"
        "4) <h2>商品ごとの詳細</h2> … 商品ごとに <h3>1. 商品カテゴリ</h3> を作り、"
        "product_slot プレースホルダー → 2〜3 文の説明 → <ul> で「価格帯」「こんな人に」\n"
    ...
    "product_block_spec": (
        "【重要】ASIN・Amazon商品URL・商品画像URLを絶対に自分で作らない・推測しない。"
        "実在しないASIN（例: B0001XXXXA のような連番）を書くと、公開後にリンク切れ・"
        "商品が表示されない不具合になる。これは内部リンクの実在URLルールと同じ扱い。\n\n"
        "商品は3〜5点、次の形でプレースホルダーとして書く（本文中の product_slot は"
        "そのまま出力し、公開前に人（または商品検索ステップ）が実在ASINに置き換える）:\n"
        "<!-- product_slot: 商品カテゴリ=\"<検索に使える具体的な商品カテゴリ、例: "
        "浴室鏡 交換用 曇り止め>\" 想定価格帯=\"<〇〇円台〜>\" 特徴=\"<1文>\" -->\n"
        "<p>[この商品ブロックは公開前に実在の商品情報に差し替えてください]</p>\n\n"
        "早見表・商品ごとの詳細のどちらも、この product_slot コメント＋プレースホルダー段落を"
        "商品の数だけ並べる。リンクタグ(<a href>)・画像タグ(<img>)は一切書かない。"
        "価格は「〇〇円台〜」の幅表記を想定として product_slot 内に書く。"
